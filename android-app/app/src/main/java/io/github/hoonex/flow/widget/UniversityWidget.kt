...
private data class WidgetRuntime(
    val source: FlowWidgetSource,
...
private fun widgetRuntime(context: Context, id: GlanceId): WidgetRuntime {
    ...
    val source = if (config.source != FlowWidgetSource.AUTO) {
        ...
        when (FlowModeStore(context).load()) {
            ...
            null -> if (SchoolStore(context).loadSelection() != null) {
                FlowWidgetSource.SCHOOL
            } else {
                FlowWidgetSource.UNIVERSITY
            }
        }
    }
...
class UniversityWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        if (runtime.source == FlowWidgetSource.SCHOOL) {
            ...
            provideContent {
                WidgetShell {
                    Text("SCHOOL", style = TextStyle(color = WidgetAccent, fontSize = 10.sp, fontWeight = FontWeight.Bold))
                    Spacer(GlanceModifier.height(7.dp))
                    Text(headline, style = TextStyle(color = WidgetText, fontSize = 19.sp, fontWeight = FontWeight.Bold))
                    if (runtime.showContext) {
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        if (runtime.source == FlowWidgetSource.SCHOOL) {
            ...
            provideContent {
                WidgetShell {
                    Text(
                        "TODAY · ${selection?.school?.name ?: "FLOW SCHOOL"}",
                    ...
                    if (runtime.showContext) {
                        ...
                        if (classes.isEmpty()) {
                            Text(
                                "${selection?.grade ?: "-"}학년 ${selection?.className ?: "-"}반",
                                style = TextStyle(color = WidgetMuted, fontSize = 11.sp)
                            )
                        } else {
                            classes.take(3).forEach { item ->
                                Text("${item.period}교시  ${item.subject}", style = TextStyle(color = WidgetMuted, fontSize = 11.sp))
...
class UniversityWeekWidget : GlanceAppWidget() {
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        if (runtime.source == FlowWidgetSource.SCHOOL) {
            ...
            val density = dashboard?.timetable.orEmpty()
                .groupBy { it.date }
                .toSortedMap()
                .entries
                .take(5)
                .joinToString("  ·  ") { (date, periods) -> "${weekday(date)} ${periods.size}" }
                .ifBlank { "주간 시간표 미등록" }
            val total = dashboard?.timetable?.size ?: 0
            provideContent {
                WidgetShell {
                    Text("SCHOOL WEEK", style = TextStyle(color = WidgetAccent, fontSize = 10.sp, fontWeight = FontWeight.Bold))
                    Spacer(GlanceModifier.height(7.dp))
                    Text(
                        if (dashboard == null) "학교 데이터를 열어주세요" else "이번 주 ${total}교시",
                        style = TextStyle(color = WidgetText, fontSize = 18.sp, fontWeight = FontWeight.Bold)
                    )
                    if (runtime.showContext) {
        ...
        val density = timetable?.let { table ->
            days.mapIndexed { index, day -> "$day ${table.classesForDay(index).size}" }.joinToString("  ·  ")
        } ?: "시간표 미등록"
        val credits = timetable?.totalCredits() ?: 0.0
        val minutes = timetable?.weeklyMinutes() ?: 0
        provideContent {
            WidgetShell {
                Text("UNIVERSITY WEEK", style = TextStyle(color = WidgetAccent, fontSize = 10.sp, fontWeight = FontWeight.Bold))
                Spacer(GlanceModifier.height(7.dp))
                Text(
    ...
    override suspend fun provideGlance(context: Context, id: GlanceId) {
        ...
        if (runtime.source == FlowWidgetSource.SCHOOL) {
            ...
            provideContent {
                MiniShell {
                    Text(
                        "SCHOOL · ${first?.let { "${it.period}교시" } ?: "TODAY"}",
                        style = TextStyle(color = WidgetAccent, fontSize = 9.sp, fontWeight = FontWeight.Bold)
                    )
                    Spacer(GlanceModifier.height(4.dp))
                    Text(
                        first?.subject ?: if (classes.isEmpty()) "수업 없음" else "${classes.size}개 수업",
...
@Composable
private fun MiniShell(content: @Composable () -> Unit) {
    Column(
        modifier = GlanceModifier
            .fillMaxSize()
            .background(WidgetBackground)
...
private fun trim(value: Double): String = if (value % 1.0 == 0.0) {
...
private fun weekday(raw: String): String = runCatching {
    LocalDate.parse(raw, DateTimeFormatter.BASIC_ISO_DATE)
        .dayOfWeek
        .getDisplayName(java.time.format.TextStyle.SHORT, Locale.KOREAN)
}.getOrDefault(raw)
...
class UniversityWidgetReceiver : GlanceAppWidgetReceiver() {
    ...
    override fun onDeleted(context: Context, appWidgetIds: IntArray) {
        clearWidgetPrefs(context, appWidgetIds)

...
async function phone(name,fixedEpoch,expectedPhase,expectedAmbient){
  ...
  try{
    await ready(page);let s=await state(page);
    assert(s.phase===expectedPhase,`${name}: wrong ambient phase ${JSON.stringify(s)}`);
    assert(s.ambientA.toLowerCase()===expectedAmbient,`${name}: expected visible School ambient palette ${JSON.stringify(s)}`);
    assert(s.bodyBackground.includes('radial-gradient')&&s.bodyBackground.includes('linear-gradient'),`${name}: ambient background is not rendered ${s.bodyBackground}`);
    assert(s.mainBackground==='rgba(0, 0, 0, 0)',`${name}: product main still blocks ambient ${s.mainBackground}`);
    assert(s.dock&&Math.abs(s.dock.center-206)<=1.5,`${name}: date deck is not centered ${JSON.stringify(s.dock)}`);
    assert(s.school.rect&&s.school.rect.width<=96&&s.school.background!=='rgba(0, 0, 0, 0)'&&s.school.shadow!=='none',`${name}: school identity still reads as stretched/flat ${JSON.stringify(s.school)}`);
    assert(s.toggle.visible&&s.toggle.buttons.length===2&&Math.abs(s.toggle.buttons[0].width-s.toggle.buttons[1].width)<=1,`${name}: Today/Week button widths diverged ${JSON.stringify(s.toggle)}`);
    assert(s.edit&&s.edit.width>=80&&s.edit.width<=90&&s.share&&s.share.width>=48&&s.share.width<=56,`${name}: utility widths are stretched ${JSON.stringify({edit:s.edit,share:s.share})}`);
    assert(s.help.background==='rgba(0, 0, 0, 0)'&&s.help.border==='0px'&&s.help.rect.height>=44,`${name}: help disclosure still looks like a card ${JSON.stringify(s.help)}`);
    assert(s.toggle.lens.display!=='none'&&s.toggle.lens.transition!=='0s',`${name}: segmented selection lens is not animated ${JSON.stringify(s.toggle.lens)}`);
    await page.screenshot({path:`${OUT}/${name}-today.png`,fullPage:false});
    if(name.includes('evening')){const before=s.toggle.lens.transform;await page.locator('.timetable-mode-toggle [data-timetable-mode="week"]').click();await page.waitForFunction(()=>document.body.classList.contains('flow-inline-week-active'));await page.waitForTimeout(80);s=await state(page);assert(s.toggle.lens.transform!==before,`${name}: selection lens did not move ${JSON.stringify(s.toggle.lens)}`);assert(s.weekAnimation.includes('flow-school-timetable-enter'),`${name}: Week content has no transition ${s.weekAnimation}`);await page.screenshot({path:`${OUT}/${name}-week.png`,fullPage:false})}
...
await phone('phone-412-evening',Date.parse('2026-09-03T09:49:00.000Z'),'evening','#e5d9ff');
{
  ...
  try{
    await ready(page);const s=await state(page);
    assert(!s.toggle.visible,`desktop: duplicate Today/Week segmented control is visible ${JSON.stringify(s.toggle)}`);
    assert(s.desktop.sidebarVisible&&s.desktop.sidebar&&s.desktop.sidebar.width>=190,`desktop: persistent navigation rail missing ${JSON.stringify(s.desktop)}`);
    assert(s.desktop.weekVisible&&s.desktop.week&&s.desktop.week.height>=44&&s.desktop.weekSmallVisible,`desktop: Week destination lost desktop hierarchy ${JSON.stringify(s.desktop)}`);
    assert(s.help.background==='rgba(0, 0, 0, 0)'&&s.help.border==='0px',`desktop: help disclosure regressed ${JSON.stringify(s.help)}`);
    await page.screenshot({path:`${OUT}/desktop-1280-today.png`,fullPage:false});
    assert(errors.length===0,`desktop: browser errors ${JSON.stringify(errors)}`);
  }finally{await context.close()}
}
await browser.close();
console.log(JSON.stringify({ok:true,coverage:['visible day/evening ambience','centered phone date deck','compact school identity','intrinsic timetable utilities','animated phone Today/Week control','text-only timetable disclosure','desktop persistent navigation rail','desktop duplicate-toggle removal']},null,2));

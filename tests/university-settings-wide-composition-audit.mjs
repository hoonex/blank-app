...
for(const c of CASES){
  ...
  try{
    ...
    console.log(`university ${c.name}: PASS`);
  }catch(error){
    report.failures.push({name:c.name,message:error?.stack||String(error)});
    console.error(`university ${c.name}: FAIL\n${error?.stack||error}`);
  }finally{await context.close()}
}

await browser.close();
await writeFile(`${OUT}/university-report.json`,JSON.stringify(report,null,2));
if(report.failures.length)throw new Error(`University settings wide composition audit found ${report.failures.length} failure(s)`);

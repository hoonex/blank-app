...
async function readState(page){return page.evaluate(()=>{
  ...
  const timetable=document.querySelector('#timetable'),rows=[...(timetable?.querySelectorAll('.period-button')||[])],dock=document.querySelector('#flowTodayDateDock'),focus=dock?.querySelector('.flow-date-focus'),school=document.querySelector('#mobileSchoolBtn'),hero=document.querySelector('#schoolHero'),feed=document.querySelector('#flowExamFeedV3');
  const dateDays=[...(dock?.querySelectorAll('.flow-date-day')||[])].filter(visible).map(node=>{const r=node.getBoundingClientRect(),s=getComputedStyle(node);return{active:node.dataset.active,offset:node.dataset.offset,iso:node.dataset.iso,width:r.width,height:r.height,left:r.left,right:r.right,radius:s.borderRadius,text:node.textContent.trim()}});
  const rowRects=rows.map(node=>{const r=node.getBoundingClientRect(),s=getComputedStyle(node);return{top:r.top,bottom:r.bottom,height:r.height,bg:s.backgroundColor,image:s.backgroundImage,shadow:s.boxShadow,radius:s.borderRadius}});
  ...
  return{
    ...
    dock:{box:box(dock),focus:box(focus),display:dock?getComputedStyle(dock).display:null,days:dateDays,picker:document.querySelector('#datePicker')?.value||''},dayStrip:document.querySelector('#dayStrip')?getComputedStyle(document.querySelector('#dayStrip')).display:null,
...
function assertBase(state,{dateCount,tabletUtilities=false}={}){
  assert(state.topbarMode==='ready',`Today live date rail mode missing ${state.topbarMode}`);
  ...
  assert(state.dock.display!=='none'&&state.dock.box?.width>=180&&state.dock.box?.height<=60&&state.dock.days.length===dateCount,`live date rail density or compact geometry incorrect ${JSON.stringify(state.dock)}`);
  const active=state.dock.days.find(day=>day.active==='true'),near=state.dock.days.find(day=>day.offset==='1')||state.dock.days.find(day=>day.offset==='-1');
  assert(active&&near&&active.height>=near.height+1&&active.width>=near.width+1,`selected date lacks center emphasis ${JSON.stringify(state.dock.days)}`);
  const activeCenter=(active.left+active.right)/2,focusCenter=((state.dock.focus?.left||0)+(state.dock.focus?.right||0))/2;
  assert(state.dock.focus&&Math.abs(activeCenter-focusCenter)<=1.5,`selected date is not centered in the live focus lens ${JSON.stringify({active,focus:state.dock.focus})}`);assertSquircle(active,'active date');
...
async function dragDate(page,dx){
  const dock=page.locator('#flowTodayDateDock'),picker=page.locator('#datePicker'),before=await picker.inputValue(),box=await dock.boundingBox();assert(box,'date rail geometry missing');const x=box.x+box.width/2,y=box.y+Math.min(40,box.height/2);
...
const portrait=await portraitContext.newPage(),portraitErrors=await prepare(portrait),initial=await readState(portrait);assertBase(initial,{dateCount:5,tabletUtilities:true});assert(initial.android==='true'&&(initial.lens===null||initial.lens==='none'),`Android Optical stability regressed ${JSON.stringify({android:initial.android,lens:initial.lens})}`);assert(!portraitErrors.length,`portrait browser errors ${JSON.stringify(portraitErrors)}`);await portrait.screenshot({path:`${OUT}/portrait-initial.png`,fullPage:true});
await portrait.waitForTimeout(5000);const stable=await readState(portrait);assertBase(stable,{dateCount:5,tabletUtilities:true});assert(stable.exams.visible===3&&JSON.stringify(stable.exams.items.map(item=>item.title))===JSON.stringify(initial.exams.items.map(item=>item.title)),`exam feed changed before user scroll`);assert(requestedDashboardDates.length===1,`future exam data fetched before user scroll ${JSON.stringify(requestedDashboardDates)}`);await portrait.screenshot({path:`${OUT}/portrait-after-5s.png`,fullPage:true});
...
const landscapeContext=await browser.newContext({viewport:{width:1024,height:768},hasTouch:true,locale:'ko-KR',timezoneId:'Asia/Seoul',userAgent:'Mozilla/5.0 (Linux; Android 14; SM-T735N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36'}),landscape=await landscapeContext.newPage(),landscapeErrors=await prepare(landscape),landscapeState=await readState(landscape);assertBase(landscapeState,{dateCount:5,tabletUtilities:true});assert(landscapeState.dock.box?.width>=420&&landscapeState.dock.box?.width<=560&&landscapeState.dock.box?.height<=60,`landscape live date rail proportions regressed ${JSON.stringify(landscapeState.dock.box)}`);assert(!landscapeErrors.length,`landscape browser errors ${JSON.stringify(landscapeErrors)}`);await landscape.screenshot({path:`${OUT}/landscape.png`,fullPage:true});await landscapeContext.close();

const phoneContext=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,locale:'ko-KR',timezoneId:'Asia/Seoul',userAgent:'Mozilla/5.0 (Linux; Android 14; SM-S931N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Mobile Safari/537.36'}),phone=await phoneContext.newPage(),phoneErrors=await prepare(phone),phoneState=await readState(phone);assertBase(phoneState,{dateCount:3});assert(phoneState.dock.box?.width>=120,`phone live date rail disappeared ${JSON.stringify(phoneState.dock.box)}`);assert(!phoneErrors.length,`phone browser errors ${JSON.stringify(phoneErrors)}`);await phone.screenshot({path:`${OUT}/phone.png`,fullPage:false});await phoneContext.close();

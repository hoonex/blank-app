...
async function readState(page){
  return page.evaluate(()=>{
    ...
    return{
      ...
      rowRects,
      examTitle:document.querySelector('.upcoming-card .card-heading h2')?.textContent,
      exams:[...(eventList?.querySelectorAll(':scope > .flow-exam-card h3')||[])].map(node=>node.textContent),
      examText:eventList?.textContent||'',visible:eventList?.dataset.flowExamVisible,count:eventList?.dataset.flowExamCount,
      examCards,peek:{display:before.display,opacity:before.opacity,height:before.height},
...
function assertUnifiedState(state,{portrait=false,expectNearest=true}={}){
  ...
  assert(state.rowRects.every(row=>row.bg!=='rgba(0, 0, 0, 0)'&&row.bg!=='transparent'&&row.shadow!=='none'),`timetable cells lost their separated surfaces ${JSON.stringify(state.rowRects)}`);
...
await portrait.mouse.move(box.x+box.width/2,box.y+70);await portrait.mouse.down();await portrait.mouse.move(box.x+box.width/2,box.y-10,{steps:8});
const duringDrag=await stackGeometry();
assert(duringDrag.length===3,`drag stack card count changed ${JSON.stringify(duringDrag)}`);
assert(duringDrag[0].top<beforeDrag[0].top-20,`front card did not track drag ${JSON.stringify({beforeDrag,duringDrag})}`);
assert(duringDrag[1].top<beforeDrag[1].top-40&&duringDrag[2].top<beforeDrag[2].top-40,`rear cards did not advance with drag ${JSON.stringify({beforeDrag,duringDrag})}`);

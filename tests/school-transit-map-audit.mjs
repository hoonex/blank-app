...
async function installKakaoFixture(page){
  await page.addInitScript(()=>{
    ...
    class Polyline{constructor({map,path,strokeColor,strokeWeight}){this.map=map;const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('viewBox','0 0 100 100');svg.classList.add('fixture-route-line');svg.style.position='absolute';svg.style.inset='0';svg.style.width='100%';svg.style.height='100%';svg.style.zIndex='1';svg.style.pointerEvents='none';const line=document.createElementNS('http://www.w3.org/2000/svg','polyline');line.setAttribute('fill','none');line.setAttribute('stroke',strokeColor||'#1769e0');line.setAttribute('stroke-width',String(Math.max(1,(strokeWeight||5)/2)));line.setAttribute('stroke-linecap','round');line.setAttribute('stroke-linejoin','round');line.setAttribute('points',path.map(p=>{const q=pos(p);return`${parseFloat(q.left)},${parseFloat(q.top)}`}).join(' '));svg.append(line);map.container.append(svg)}setMap(next){if(!next)this.map=null}}
    class CustomOverlay{constructor({map,position,content,xAnchor=.5,yAnchor=.5}){this.map=map;this.content=content;content.classList.add('fixture-overlay');content.style.position='absolute';content.style.zIndex='2';const q=pos(position);content.style.left=q.left;content.style.top=q.top;content.style.transform=`translate(${-xAnchor*100}%,${-yAnchor*100}%)`;map.container.append(content)}setMap(next){if(!next)this.content?.remove();this.map=next}}

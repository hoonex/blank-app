...
async function installKakaoFixture(page){
  await page.addInitScript(()=>{
    ...
    class CustomOverlay{constructor({map,position,content}){this.map=map;this.content=content;content.classList.add('fixture-overlay');content.style.position='absolute';const q=pos(position);content.style.left=q.left;content.style.top=q.top;map.container.append(content)}setMap(next){if(!next)this.content?.remove();this.map=next}}
...
for(const testCase of cases){
  ...
  await page.locator('[data-transit-map-close]').click();await page.waitForFunction(()=>!document.querySelector('.flow-transit-map-shell'));

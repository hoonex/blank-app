...
const officialPath=[
  {x:128.695,y:35.876},{x:128.698,y:35.875},{x:128.702,y:35.873},{x:128.707,y:35.869},{x:128.711,y:35.867},{x:128.714,y:35.866},
  {x:128.691,y:35.872},{x:128.666,y:35.876},{x:128.642,y:35.878},{x:128.629,y:35.879},{x:128.625,y:35.882},{x:128.623,y:35.883},{x:128.624,y:35.885},{x:128.626,y:35.886},
];
const cases=[
...
async function baseFixture(page,mode,counters){
  ...
  if(mode==='rail'){
    ...
    await page.route('**/functions/v1/transit-map*',route=>{counters.map+=1;const moved=counters.map>1;return json(route,{generatedAt:new Date().toISOString(),provider:'TAGO-public-data+Daegu-official-SHP',geometry:'daegu-official-bus-link-snapshot',geometrySnapshot:'2025-09-03',cityCode:'22',route:{id:'DGB708',no:'708',start:routeStops[0],end:routeStops.at(-1),stops:routeStops,path:officialPath,officialGeometry:{matchedStops:routeStops.length,routedPairs:routeStops.length-1,maxSnapMeters:12.4}},vehicles:[{vehicleNo:'대구70자1234',nodeId:moved?'DGB7010003':'DGB7010002',nodeName:moved?'반야월역':'각산역',nodeOrder:moved?12:11,x:moved?128.713:128.707,y:moved?35.866:35.869}],vehicleStatus:'live'})});
  }
...
}
{
  ...
  const traceState=await page.evaluate(()=>({routeTrace:document.querySelector('.flow-transit-map-shell')?.dataset.routeTrace||'',geometrySnapshot:document.querySelector('.flow-transit-map-shell')?.dataset.geometrySnapshot||'',tracePoints:Number(document.querySelector('.flow-transit-map-shell')?.dataset.routeTracePoints||0),stopCount:Number(document.querySelector('.flow-transit-map-shell')?.dataset.stopCount||0),note:document.querySelector('.flow-transit-map-note')?.textContent?.trim()||'',status:document.querySelector('.flow-transit-map-status')?.textContent?.trim()||'',polylinePoints:Math.max(0,...[...document.querySelectorAll('.flow-transit-map-canvas polyline')].map(node=>Number(node.dataset.pathPoints||0)))}));
  if(traceState.routeTrace!=='official-road-geometry'||traceState.geometrySnapshot!=='2025-09-03'||traceState.tracePoints<=traceState.stopCount||traceState.polylinePoints<=traceState.stopCount||!traceState.note.includes('공식 버스 노선 공간정보')||!traceState.status.includes('공식 도로 경로'))throw new Error(`official bus road geometry contract failed ${JSON.stringify(traceState)}`);

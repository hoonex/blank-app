...
const universityContext=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true,locale:'ko-KR',timezoneId:'Asia/Seoul'});const university=await universityContext.newPage();await universityFixtures(university);await university.goto(`${BASE}/university/`,{waitUntil:'domcontentloaded'});await university.locator('#appView:not(.hidden)').waitFor();const universityState=await inspect(university,'.bottom-nav',5);validate('University Optical nav',universityState,{count:5,first:'오늘',last:'설정'});await university.screenshot({path:`${OUT}/university-optical-nav.png`,fullPage:false});await universityContext.close();

await browser.close();console.log(JSON.stringify({schoolState,universityState},null,2));

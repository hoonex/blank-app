...
// Settings is a shared document-flow destination under the floating nav, with working ambience and haptics.
...
let settings=await page.evaluate(()=>{const panel=document.querySelector('#flowSchoolSettingsView'),style=panel?getComputedStyle(panel):null,pr=panel?.getBoundingClientRect();return{panelBottom:pr?.bottom||0,viewport:innerHeight,paddingBottom:parseFloat(style?.paddingBottom)||0,position:style?.position||'',overflowY:style?.overflowY||'',panelScrollTop:panel?.scrollTop||0,windowScrollY:window.scrollY}});
assert(settings.position==='relative'&&settings.overflowY==='visible'&&settings.panelScrollTop===0,`Settings is not using the shared document-flow shell ${JSON.stringify(settings)}`);assert(settings.paddingBottom>=120,`Settings content lacks bottom-nav clearance ${JSON.stringify(settings)}`);
...
await page.evaluate(()=>window.scrollTo({top:document.documentElement.scrollHeight,behavior:'auto'}));await page.waitForTimeout(100);const endState=await page.evaluate(()=>{const panel=document.querySelector('#flowSchoolSettingsView'),nav=document.querySelector('#bottomNav'),cards=[...panel.querySelectorAll('.flow-settings-card')].filter(node=>getComputedStyle(node).display!=='none'),last=cards.at(-1),lr=last?.getBoundingClientRect(),nr=nav?.getBoundingClientRect();return{lastBottom:lr?.bottom||0,navTop:nr?.top||0,windowScrollY:window.scrollY,scrollHeight:document.documentElement.scrollHeight,clientHeight:innerHeight,panelScrollTop:panel.scrollTop}});
assert(endState.windowScrollY>0&&endState.panelScrollTop===0&&endState.lastBottom<=endState.navTop-6,`last Settings content cannot clear the floating nav in document flow ${JSON.stringify(endState)}`);
...
console.log(JSON.stringify({ok:true,date:'multi-day kinetic inertia + magnetic snap',exams:'1 detailed + 3 compact + rear peek with scroll promotion',settings:'shared document-flow surface + visible ambient + haptics',reload:'no legacy first-frame flash'},null,2));
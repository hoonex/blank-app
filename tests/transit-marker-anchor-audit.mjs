...
const geometry=await page.evaluate(()=>{
  ...
  return {
    target:sr&&{x:sr.left+320,y:sr.top+240},
...
await browser.close();
if(!geometry.target||!geometry.pin||!geometry.dot||!geometry.label)throw new Error(`Transit marker DOM missing: ${JSON.stringify(geometry)}`);
const dx=Math.abs(geometry.dot.cx-geometry.target.x),dy=Math.abs(geometry.dot.cy-geometry.target.y);
...
console.log(JSON.stringify({ok:true,coordinate:geometry.target,dotCenter:{x:geometry.dot.cx,y:geometry.dot.cy},anchorBox:geometry.pin,labelLeft:geometry.label.left},null,2));

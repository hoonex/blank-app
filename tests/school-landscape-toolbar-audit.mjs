...
async function inspect(page,contentSelector){return page.evaluate(({contentSelector})=>{
  ...
  return{header:r(header),nav:r(nav),brand:r(brand),school:r(school),content:r(content),items,weekInNav:!!nav?.querySelector(':scope>[data-view="week"]'),position:navStyle?.position||'',navRadius:parseFloat(navStyle?.borderRadius)||0,navCorner:navStyle?.getPropertyValue('corner-shape')||'',clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,viewportHeight:innerHeight,bodyPaddingBottom:parseFloat(getComputedStyle(document.body).paddingBottom)||0,heroVisible:visible(hero),sidebarVisible:visible(sidebar),glassMode:document.documentElement.dataset.flowGlassMode||'',refraction:document.documentElement.dataset.flowGlassRefraction||''};
},{contentSelector})}
function validate(label,state){
  ...
  if(state.navRadius>20||/squircle|superellipse/i.test(state.navCorner))throw new Error(`${label}: bottom nav still uses oversized/squircle curvature ${JSON.stringify({radius:state.navRadius,corner:state.navCorner})}`);

...
async function schoolMobile(){
  ...
  await mockSchool(page);await page.goto(BASE,{waitUntil:'domcontentloaded'});await page.locator('#dashboard:not(.hidden)').waitFor();
  await page.waitForFunction(()=>document.documentElement.dataset.flowSchoolSurface==='ready'&&document.documentElement.dataset.flowSchoolRuntimeV6==='ready');await waitMaterial(page);
  ...
  const shell=await page.evaluate(()=>{const nav=document.querySelector('#bottomNav'),settings=document.querySelector('#mobileSettingsBtn'),items=[...nav.querySelectorAll(':scope > .mobile-tab')].filter(x=>getComputedStyle(x).display!=='none'),week=document.querySelector('.timetable-mode-toggle > [data-timetable-mode="week"]');return{count:items.length,settingsLast:settings===items.at(-1),transitInline:Boolean(nav.querySelector(':scope > [data-view="transit"]')),weekInBottom:Boolean(nav.querySelector(':scope > [data-view="week"]')),weekInline:Boolean(week),weekRouteAttr:week?.getAttribute('data-view')||'',grid:getComputedStyle(nav).gridTemplateColumns,width:settings.getBoundingClientRect().width,clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth}});
  if(shell.count!==5||!shell.settingsLast||!shell.transitInline||shell.weekInBottom||!shell.weekInline||shell.weekRouteAttr||shell.grid.trim().split(/\s+/).length!==5||shell.width<44||shell.scrollWidth>shell.clientWidth+3)throw new Error(`school five-slot navigation / inline Week failed ${JSON.stringify(shell)}`);

...
async function schoolMobile(){
  ...
  const shell=await page.evaluate(()=>{const nav=document.querySelector('#bottomNav'),settings=document.querySelector('#mobileSettingsBtn'),items=[...nav.querySelectorAll(':scope > .mobile-tab')].filter(x=>getComputedStyle(x).display!=='none');return{count:items.length,settingsLast:settings===items.at(-1),grid:getComputedStyle(nav).gridTemplateColumns,width:settings.getBoundingClientRect().width,clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth}});
  if(shell.count!==5||!shell.settingsLast||shell.grid.trim().split(/\s+/).length!==5||shell.width<44||shell.scrollWidth>shell.clientWidth+3)throw new Error(`school five-slot navigation failed ${JSON.stringify(shell)}`);

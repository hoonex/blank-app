...
// ID-scoped CSS is mirrored through data attributes, while the live DOM keeps unique IDs.
await page.evaluate(()=>{
...
const fidelity=await page.evaluate(()=>{
  const original=document.querySelector('.product-main>#flowRefractionFidelityProbe'),copy=document.querySelector('.flow-refraction-source-copy[data-flow-refraction-source="school-main"] [data-flow-refraction-id="flowRefractionFidelityProbe"]'),os=getComputedStyle(original),cs=getComputedStyle(copy);
  return{originalLookup:document.getElementById('flowRefractionFidelityProbe')===original,idCount:document.querySelectorAll('#flowRefractionFidelityProbe').length,color:[os.color,cs.color],fontSize:[os.fontSize,cs.fontSize]};
});
if(!fidelity.originalLookup||fidelity.idCount!==1||fidelity.color[0]!==fidelity.color[1]||fidelity.fontSize[0]!==fidelity.fontSize[1])throw new Error(`Optical copy lost ID-scoped visual fidelity or duplicated a source ID: ${JSON.stringify(fidelity)}`);
...
await page.waitForFunction(()=>document.querySelector('.flow-refraction-source-copy[data-flow-refraction-source="school-switch"] [data-flow-refraction-id="switchSearch"]'));
const sourceState=await page.evaluate(()=>{
  const actual=document.querySelector('body > #switchDialog #switchSearch');
  return{source:document.querySelector('.flow-refraction-source-copy')?.dataset.flowRefractionSource,hasSwitch:!!document.querySelector('.flow-refraction-source-copy [data-flow-refraction-id="switchSearch"]'),hasHero:!!document.querySelector('.flow-refraction-source-copy [data-flow-refraction-id="heroSchoolName"]'),switchIdCount:document.querySelectorAll('#switchSearch').length,lookupIsActual:document.getElementById('switchSearch')===actual};
});
if(sourceState.source!=='school-switch'||!sourceState.hasSwitch||sourceState.hasHero||sourceState.switchIdCount!==1||!sourceState.lookupIsActual)throw new Error(`Optical lens sampled or isolated the School switch surface incorrectly: ${JSON.stringify(sourceState)}`);
...
const scrollState=await page.evaluate(async()=>{
  ...
  dialog.scrollTop=Math.min(120,Math.max(0,dialog.scrollHeight-dialog.clientHeight));dialog.dispatchEvent(new Event('scroll'));await new Promise(requestAnimationFrame);await new Promise(requestAnimationFrame);
...
await page.waitForFunction(()=>document.querySelector('.flow-refraction-source-copy[data-flow-refraction-source="school-switch"] [data-flow-refraction-id="switchResults"] [data-result-index]'));

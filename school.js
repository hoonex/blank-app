...
function renderSchoolInfo(){
  ...
  const entries=[['설립구분',s.type],['학교 구분',s.highSchoolType||s.kind],['남녀 구분',s.coed],['주야',s.dayNight],['설립일',fmtDate8(s.founded)],['개교기념일',fmtDate8(s.anniversary)],['교육청',s.officeName],['관할기관',s.jurisdiction],['주소',[s.address,s.addressDetail].filter(Boolean).join(' ')],['전화',s.phone],['팩스',s.fax],['계열',s.highSchoolTrack||s.specialPurpose]];$('#schoolInfoGrid').innerHTML=entries.filter(([,v])=>v).map(([k,v])=>`<div class="info-tile"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('');
...
function bellDefaults(){const kind=profile?.school?.kind||'';if(kind.includes('초등'))return{start:'09:00',lesson:40,break:10};if(kind.includes('중학'))return{start:'09:00',lesson:45,break:10};return{start:'08:30',lesson:50,break:10}}
...
function saveSettings(){writeJson(BELL_KEY,{start:$('#bellStart').value||'08:30',lesson:Math.max(30,Math.min(90,Number($('#lessonMinutes').value)||50)),break:Math.max(5,Math.min(30,Number($('#breakMinutes').value)||10))});closeDialog($('#settingsDialog'));renderClock();toast('설정을 저장했습니다.')}

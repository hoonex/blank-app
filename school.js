...
function renderRank(){const total=Number($('#rankTotal')?.value||0),rank=Number($('#rankValue')||0),main=$('#rankMain'),sub=$('#rankSub');if(!main||!sub)return;if(total>0)localStorage.setItem(RANK_TOTAL_KEY,String(total));if(!(total>0&&rank>0&&rank<=total)){main.textContent='값을 입력하세요';sub.textContent='수강자수 기준';return}const pct=rank/total*100,g=pct<=10?1:pct<=34?2:pct<=66?3:pct<=90?4:5;main.textContent=`상위 ${pct.toFixed(pct<10?1:0)}% · ${g}등급 구간`;sub.textContent=`${rank} / ${total}명`}

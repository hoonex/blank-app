...
const storedBefore=await page.evaluate(()=>JSON.parse(localStorage.getItem('flow-university-dashboard-layout-v2')||'null')?.widgets?.memo?.size||'');
...
const duringResize=await page.evaluate(id=>{const e=document.querySelector(`[data-widget-id="${id}"]`);const r=e?.getBoundingClientRect();return{size:e?.dataset.size||'',stored:JSON.parse(localStorage.getItem('flow-university-dashboard-layout-v2')||'null')?.widgets?.[id]?.size||'',position:e?getComputedStyle(e).position:'',width:r?.width||0,height:r?.height||0,placeholder:document.querySelectorAll('.widget-resize-placeholder').length,target:e?.querySelector('.widget-v2-size-label')?.textContent||''}},'memo');
if(duringResize.size!==beforeSize||duringResize.stored!==storedBefore||duringResize.position!=='fixed'||duringResize.placeholder!==1)throw new Error(`Resize snapped or persisted before pointer release: ${JSON.stringify({beforeSize,storedBefore,duringResize})}`);
await page.screenshot({path:'university-audit/mobile-widget-resize-live.png',fullPage:true});
...
await page.mouse.move(start.x,start.y);await page.mouse.down();await page.waitForTimeout(120);
const longPressArmed=await page.evaluate(()=>({arming:document.querySelector('[data-widget-id="campus"]')?.classList.contains('widget-longpress-arming'),editing:document.querySelector('#todayView')?.classList.contains('dashboard-editing')}));
if(!longPressArmed.arming||longPressArmed.editing)throw new Error(`Long press tactile arming state is missing or edit mode started too early: ${JSON.stringify(longPressArmed)}`);
await page.waitForTimeout(350);
const longPressState=await page.evaluate(()=>({editing:document.querySelector('#todayView')?.classList.contains('dashboard-editing'),arming:document.querySelector('[data-widget-id="campus"]')?.classList.contains('widget-longpress-arming'),floating:document.querySelector('[data-widget-id="campus"]')?.classList.contains('widget-direct-drag'),placeholder:document.querySelectorAll('.widget-drag-placeholder').length,position:getComputedStyle(document.querySelector('[data-widget-id="campus"]')).position}));
if(!longPressState.editing||longPressState.arming||!longPressState.floating||longPressState.placeholder!==1||longPressState.position!=='fixed')throw new Error(`Long press did not lift widget cleanly: ${JSON.stringify(longPressState)}`);
await page.screenshot({path:'university-audit/mobile-widget-longpress-live.png',fullPage:true});
...
const report={schoolFirstPaint,initial,beforeSize,beforeRect,storedBefore,duringResize,afterSize,resized,longPressArmed,longPressState,dragFollow,orderBefore,orderAfter,picker,utilityState,reloaded,desktop,idleMutations,consoleErrors,pageErrors};

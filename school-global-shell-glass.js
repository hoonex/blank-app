...
function installStyle(){
  ...
  style.id=STYLE_ID;
  style.textContent=`
/* Shared School chrome contract. Phone/tablet use one edge-to-edge top shell and
   one circular pill navigation bar on every view; desktop never inherits them. */
@media(max-width:1180px){
  ...
  html[data-flow-school-ui="v2"]:not([data-flow-glass-mode="optical"]) body #dashboard:not(.hidden) #bottomNav.mobile-bottom-nav::before{
    background:
      radial-gradient(125% 110% at var(--flow-lens-light-x,34%) -8%,rgba(255,255,255,.80) 0%,rgba(255,255,255,.24) 31%,transparent 59%),
      linear-gradient(180deg,color-mix(in srgb,var(--accent) 10%,var(--surface)),color-mix(in srgb,var(--accent) 5%,var(--surface)))!important;
...
@media(min-width:1181px){
  html[data-flow-school-ui="v2"] body #dashboard:not(.hidden).product-shell{
    display:grid!important;
  }
  html[data-flow-school-ui="v2"] body #dashboard:not(.hidden) .desktop-sidebar{
    display:flex!important;

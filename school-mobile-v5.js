...
function bindDateWheel(){
  ...
  const end=event=>{if(!dateState?.dragging||event.pointerId!==dateState.id)return;event.stopImmediatePropagation();dateState.dragging=false;delete dock.dataset.kineticDragging;try{viewport.releasePointerCapture?.(event.pointerId)}catch{}if(dateState.moved){dateSuppressUntil=performance.now()+450;const totalX=dateState.lastX-dateState.startX,slot=slotWidth();if(Math.abs(totalX)>=28&&Math.round(-dateState.position/slot)===0){dateState.position=Math.sign(totalX)*slot*.56;applyDateVisual(dateState.position,true)}runDateInertia()}else magneticSnap()};

(()=>{
  ...
  let jelly=null,frame=0,lastFrame=0;
  ...
  function syncControls(){
    const on=enabled(),active=optical()&&on;
    root.dataset.flowOpticalJelly=String(active);
  ...
  function animate(ts){
    frame=0;
    if(!optical()||!enabled()||reducedMotion.matches){x=y=vx=vy=0;applyTransform();return}
  ...
  function syncInputs(){
    ensureJelly();
    if(optical()&&enabled())addInputs();else removeInputs();
  }
  ...
  function resetNeutral(){neutralBeta=neutralGamma=null;setTarget(0,0)}

  window.addEventListener('flow:glass-mode-changed',()=>{installSettings();syncControls()},{passive:true});
  window.addEventListener('orientationchange',resetNeutral,{passive:true});
  window.addEventListener('pageshow',()=>{installSettings();syncControls()},{passive:true});
  document.addEventListener('visibilitychange',()=>{if(document.hidden)removeInputs();else syncInputs()},{passive:true});
  reducedMotion.addEventListener?.('change',()=>{resetNeutral();syncControls()});

  const init=()=>{ensureStyle();ensureJelly();installSettings();syncControls()};

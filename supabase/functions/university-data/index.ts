...
const KEY_ENV_BY_SERVICE: Record<string,string> = {
...
function serviceKey(service: string) {
  const envName = KEY_ENV_BY_SERVICE[service];
  const value = envName ? (Deno.env.get(envName) || "").trim() : "";
  if (!envName || !value) throw new Error("university public-data backend is not configured");

...
function normalizeActionControls(){
  ...
  if(actions){if(toggle&&actions.firstElementChild!==toggle)actions.prepend(toggle);if(edit)actions.append(edit);if(share)actions.append(share)}
  if(edit){edit.classList.add('flow-school-utility-action');edit.dataset.flowSchoolAction='edit'}if(share){share.classList.add('flow-school-utility-action');share.dataset.flowSchoolAction='share'}if(allergy){allergy.classList.add('flow-school-utility-action');allergy.dataset.flowSchoolAction='allergy'}
}
...
function enhanceTimetable(){
  ...
  rows.forEach(row=>{const period=Number(row.dataset.period)||0,window=windows.find(item=>item.period===period),copy=row.children[1];if(!window||!copy)return;let time=$('.flow-period-time',copy);if(!time){time=document.createElement('small');time.className='flow-period-time';copy.append(time)}const active=current===period;time.textContent=`${formatMinutes(window.start)}–${formatMinutes(window.end)}${active?' · 진행 중':''}`;row.classList.toggle('flow-period-current',active);if(active)row.setAttribute('aria-current','true');else row.removeAttribute('aria-current')});
}
...
function updateQuickExam(exams){const card=$('#quickEvent')?.closest('.status-card'),first=exams[0];if(card?.querySelector('.status-label'))card.querySelector('.status-label').textContent='다음 시험';if(!first){if($('#quickEvent'))$('#quickEvent').textContent='예정 없음';if($('#quickEventSub'))$('#quickEventSub').textContent='확인된 시험 일정 없음';return}if($('#quickEvent'))$('#quickEvent').textContent=`${examDday(first.date)} ${first.name}`;if($('#quickEventSub'))$('#quickEventSub').textContent=examDateLabel(first.date)}
function renderExamStack(){
  const box=$('#eventList'),card=box?.closest('.upcoming-card');if(!box||!card)return;captureAcademicExams();const exams=upcomingExams(),signature=exams.map(item=>`${item.date}:${item.name}`).join('|');if(signature!==examSignature){examSignature=signature;examStackIndex=0}const title=$('.card-heading h2',card),kicker=$('.section-kicker',card);if(title)title.textContent='다가오는 시험';if(kicker)kicker.textContent='EXAMS';updateQuickExam(exams);box.classList.add('flow-exam-stack');box.dataset.flowExamCount=String(exams.length);examStackIndex=Math.max(0,Math.min(examStackIndex,Math.max(0,exams.length-1)));const visible=exams.slice(examStackIndex,examStackIndex+3);box.innerHTML=visible.length?visible.map((exam,index)=>examCardMarkup(exam,index)).join('')+(exams.length>1?'<span class="flow-exam-hint">위아래로 넘겨 보기</span>':''):'<div class="flow-exam-empty">다가오는 시험 일정이 없습니다.</div>';bindExamStack(box);
}
function bindExamStack(box){
  if(box.dataset.flowExamBound==='true')return;box.dataset.flowExamBound='true';let startY=0,startAt=0,dragging=false;const reset=()=>{dragging=false;box.style.setProperty('--flow-exam-drag','0px');delete box.dataset.swipe};const settle=direction=>{const list=upcomingExams();if(direction==='up'&&examStackIndex<list.length-1){box.dataset.swipe='up';setTimeout(()=>{examStackIndex++;renderExamStack();reset()},170)}else if(direction==='down'&&examStackIndex>0){box.dataset.swipe='down';setTimeout(()=>{examStackIndex--;renderExamStack();reset()},170)}else reset()};box.addEventListener('pointerdown',event=>{if(event.button!==0)return;dragging=true;startY=event.clientY;startAt=performance.now();box.setPointerCapture?.(event.pointerId)});box.addEventListener('pointermove',event=>{if(!dragging)return;const dy=Math.max(-92,Math.min(72,event.clientY-startY));box.style.setProperty('--flow-exam-drag',`${dy}px`)});const end=event=>{if(!dragging)return;const dy=event.clientY-startY,elapsed=Math.max(16,performance.now()-startAt),velocity=dy/elapsed;box.releasePointerCapture?.(event.pointerId);if(dy<-34||velocity<-.42)settle('up');else if(dy>34||velocity>.42)settle('down');else reset()};box.addEventListener('pointerup',end);box.addEventListener('pointercancel',reset);
}
function markAndroidStableGlass(){if(/Android/i.test(navigator.userAgent||''))document.documentElement.dataset.flowAndroidStableGlass='true'}

...
function syncExternalLayout(e){if(e?.detail)e.detail.handled=true;saveState();refreshMemoPicker()}
function init(){injectStyles();const wait=()=>{if(!getGrid()){setTimeout(wait,80);return}buildMemo();loadState();installAllControls();installColumnControl();applyState();document.addEventListener('flow:dashboard-layout-sync',syncExternalLayout);document.addEventListener('click',e=>{if(e.target.closest('#widgetAddBtn,[data-picker-id],#widgetResetBtn,#widgetDoneBtn'))syncAfterLegacyAction()},{passive:true});window.addEventListener('resize',()=>{if(!state||resizeSession||dragSession)return;applyColumns()},{passive:true})};wait()}

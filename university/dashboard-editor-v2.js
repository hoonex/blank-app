...
function cancelPress(removeEnd=true){if(!pressSession)return;clearTimeout(pressSession.timer);pressSession=null;document.removeEventListener('pointermove',pressMove);if(removeEnd){document.removeEventListener('pointerup',pressEnd);document.removeEventListener('pointercancel',pressEnd)}}
function beginDrag(el,pointerId,x,y,fromLongPress=false){
  ...
  const rect=el.getBoundingClientRect(),placeholder=makePlaceholder(el,'drag'),offsetX=Math.max(0,Math.min(rect.width,x-rect.left)),offsetY=Math.max(0,Math.min(rect.height,y-rect.top));
  dragSession={el,pointerId,placeholder,grid,offsetX,offsetY,lastX:x,lastY:y,fromLongPress,moved:fromLongPress,startX:x,startY:y};el.classList.add('widget-dragging');el.dataset.directDragging='1';try{el.setPointerCapture?.(pointerId)}catch{}liftWidget(el,rect,'drag');moveFloatingDrag(x,y);document.addEventListener('pointermove',dragMove,{passive:false});document.addEventListener('pointerup',endDrag,{once:true});document.addEventListener('pointercancel',endDrag,{once:true})
}
...
function visibleDropTargets(s){return getWidgets().filter(el=>el!==s.el&&!el.classList.contains('widget-hidden')&&el.isConnected)}
function targetForPoint(s,x,y){const direct=document.elementFromPoint(x,y)?.closest?.('#widgetDashboard [data-widget-id]');if(direct&&direct!==s.el&&!direct.classList.contains('widget-hidden'))return direct;let best=null,bestD=Infinity;for(const el of visibleDropTargets(s)){const r=el.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,d=Math.hypot((x-cx)*.8,y-cy);if(d<bestD){bestD=d;best=el}}return bestD<240?best:null}
function dragMove(e){const s=dragSession;if(!s||e.pointerId!==s.pointerId)return;e.preventDefault();s.lastX=e.clientX;s.lastY=e.clientY;if(!s.moved&&Math.hypot(e.clientX-s.startX,e.clientY-s.startY)>5)s.moved=true;moveFloatingDrag(e.clientX,e.clientY);const target=targetForPoint(s,e.clientX,e.clientY);if(!target)return;const r=target.getBoundingClientRect(),before=e.clientY<r.top+r.height/2||(Math.abs(e.clientY-(r.top+r.height/2))<r.height*.25&&e.clientX<r.left+r.width/2);s.grid.insertBefore(s.placeholder,before?target:target.nextSibling)}

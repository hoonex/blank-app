...
function applyDensity(next,{focalY=null,persist=false}={}){const grid=getGrid(),geometry=getScaleGeometry(grid);density=clamp(Number(next)||1,DENSITY_MIN,DENSITY_MAX);if(!grid||!geometry){if(persist)persistDensity();return false}const oldHour=parseFloat(getComputedStyle(grid).getPropertyValue('--hour'))||baseHourPx(),newHour=baseHourPx()*density,bodyRect=geometry.body.getBoundingClientRect();let anchorMinutes=null;if(Number.isFinite(focalY)&&oldHour>0){anchorMinutes=clamp(geometry.start+((focalY-bodyRect.top)/oldHour)*60,geometry.start,geometry.end)}grid.style.setProperty('--hour',`${newHour}px`);const totalHeight=(geometry.spanMinutes/60)*newHour;geometry.body.style.height=`${totalHeight}px`;for(const label of geometry.labels){const minute=timeMinutes(label.textContent);if(Number.isFinite(minute))label.style.top=`${((minute-geometry.start)/60)*newHour}px`}grid.querySelectorAll('.day-column').forEach(column=>{column.style.height=`${totalHeight}px`;column.querySelectorAll('.course-block').forEach(block=>{const time=courseTime(block);if(!time)return;block.style.top=`${((time.start-geometry.start)/60)*newHour}px`;block.style.height=`${Math.max(22,((time.end-time.start)/60)*newHour)}px`})});updateNowLine(grid,geometry.start,newHour);grid.dataset.timetableDensity=String(Math.round(density*100));grid.dataset.timetableHourPx=String(Number(newHour.toFixed(2)));if(anchorMinutes!==null&&geometry.scroll){const nextBodyTop=geometry.body.getBoundingClientRect().top,targetY=nextBodyTop+((anchorMinutes-geometry.start)/60)*newHour;geometry.scroll.scrollTop+=targetY-focalY}if(persist)persistDensity();window.dispatchEvent(new CustomEvent('flow:timetable-density-changed',{detail:{scale:density,hourPx:newHour}}));return true}
...
window.addEventListener('flow:timetable-changed',()=>scheduleReapply(90));
